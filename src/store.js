...
export default class Store {
  ...
  add(data) {
    this.rows = this.rows.push(data);
  }

  delete(data) {
    this.rows = this.rows.filterNot(r => data.has(r));
  }

  load() {
    this.rows = this.rows.push(...Store.data);
    /*
    const data = [
...
//see: https://github.com/benoitvallon/100-best-books/blob/master/books.json
Store.data = [
  {
